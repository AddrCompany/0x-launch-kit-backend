...
class OrderBook {
    ...
    static async onOrderLifeCycleEventAsync(lifecycleEvent, orders) {
        ...
        if (lifecycleEvent === types_2.OrderWatcherLifeCycleEvents.Add) {
            ...
            const orderHashes = orders.map(_0x_js_1.orderHashUtils.getOrderHashHex);
    ...
    async addExistingOrdersToOrderWatcherAsync() {
        ...
        if (rejected.length > 0) {
            await OrderBook.onOrderLifeCycleEventAsync(
                types_2.OrderWatcherLifeCycleEvents.Remove,
                rejected.map(r => r.order),
            );
        }
